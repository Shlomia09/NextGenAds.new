...
serve(async (req) => {
  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      insightsUrl.searchParams.set('fields', 'spend,impressions,clicks,cpm,cpc,actions,action_values,purchase_roas');
      ...
      // Parse purchases
      const purchaseAction = (insight.actions || []).find((a: { action_type: string }) => a.action_type === 'offsite_conversion.fb_pixel_purchase');
      const purchases = parseInt(purchaseAction?.value || '0');

      // Parse revenue
      const revenueAction = (insight.action_values || []).find((a: { action_type: string }) => a.action_type === 'offsite_conversion.fb_pixel_purchase');
      const revenue = parseFloat(revenueAction?.value || '0');

      // Parse ROAS
      const roasData = (insight.purchase_roas || []).find((a: { action_type: string }) => a.action_type === 'offsite_conversion.fb_pixel_purchase');
      const roas = parseFloat(roasData?.value || '0');
